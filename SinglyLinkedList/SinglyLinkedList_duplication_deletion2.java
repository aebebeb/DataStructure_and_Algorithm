...
// 시간이 많이 걸리지만 임의의 노드의 값을 저장하여 노드를 순회하며 데이터 값을 비교하고 같은 데이터 값을 가지고 있는 노드는 제거한다.
	...
	{	
		...
		while (stan.next != null) {
			// 헤드와 모든 노드의 값 비교 -> 그 다음 노드의 값과 다른 노드들의 값 비교 ... 마지막 - 1 번째 노드의 값과 마지막 노드의 값 비교, 값 비교 끝
			...
			while(cur.next != null) {
				...
				// 말단 노드 -1와 말단 노드의 값 비교해서 일치하면 삭제
				else if (stan.next.next == null && stan.data == cur.next.data) {
					cur.next = null;
				}
	...
	public class SinglyLinkedList_duplication_deletion2 {

		public static void main(String[] args) {
			LinkedListStructure wh = new LinkedListStructure();
			wh.append(1);
			wh.append(2);
			wh.append(3);
			wh.append(4);
